...
# Initialize LangSmith tracing for Streamlit
@st.cache_resource
def init_langsmith():
    """Initialize LangSmith tracing (cached to avoid re-initialization)."""
    client = initialize_langsmith()
    if client:
        logger.info("LangSmith tracing initialized for Streamlit app")
        return True
    else:
...
def init_session_state():
    ...
    if "messages" not in st.session_state:
        st.session_state.messages = [
            {
                "role": "assistant",
                "content": "Hello! I'm Inventra, your AI assistant for inventory and financial management. How can I help you today?",
            }
        ]
...
def render_chat_interface():
    ...
    st.markdown(
        "<div class='main-header'>Inventra AI Assistant</div>", unsafe_allow_html=True
    )
    st.markdown(
        "<div class='sub-header'>Ask me anything about inventory, sales, finances, or get recommendations</div>",
        unsafe_allow_html=True,
    )
    ...
    prompt = st.chat_input(
        "Ask about inventory, sales, finances, or get recommendations..."
    )
...
def render_data_explorer():
    ...
    with col1:
        ...
        if inv.get("region_summary"):
            df = pd.DataFrame(
                [{"Region": k, "Quantity": v} for k, v in inv["region_summary"].items()]
            )
    ...
    if inv.get("low_stock_items"):
        ...
        st.dataframe(
            low_stock_df[
                ["sku", "name", "category", "region", "qty", "reorder_threshold"]
            ],
...
def render_ticket_manager():
    ...
    if tickets:
        ...
        st.dataframe(
            tickets_df[available_cols], use_container_width=True, hide_index=True
        )
        ...
        with col2:
            new_status = st.selectbox(
                "New Status", ["pending", "approved", "rejected", "completed"]
            )
...
def render_forecast_accuracy():
    ...
    if report["recent_forecasts"]:
        ...
        if not recent_df.empty:
            ...
            st.dataframe(
                recent_df[available_cols], use_container_width=True, hide_index=True
            )
    else:
        st.info(
            "No forecast data available yet. Make some recommendations to start tracking accuracy!"
        )
    ...
    if total_forecasts > 0 and avg_accuracy is not None:
        ...
        if avg_accuracy >= 80:
            st.success(
                f"🎉 Excellent forecast accuracy at {avg_accuracy:.1f}%! The system is performing well."
            )
        elif avg_accuracy >= 60:
            st.info(
                f"📊 Good forecast accuracy at {avg_accuracy:.1f}%. Room for improvement by refining weather-demand correlations."
            )
        else:
            st.warning(
                f"⚠️ Forecast accuracy is {avg_accuracy:.1f}%. Consider reviewing the prediction models and weather impact factors."
            )
...
def main():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["💬 Chat Assistant", "📊 Data Explorer", "📝 Tickets", "🎯 Forecast Accuracy"]
    )
