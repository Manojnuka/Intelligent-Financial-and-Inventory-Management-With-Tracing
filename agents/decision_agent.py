...
def build_sales_opportunity_prompt(
    sales_data: Dict[str, Any], category: Optional[str] = None, days: int = 7
) -> str:
...
def build_vendor_selection_prompt(
    vendors: List[Dict[str, Any]], sku: Optional[str] = None
) -> str:
...
def build_financial_health_prompt(
    finance: Dict[str, Any], region: Optional[str] = None
) -> str:
    ...
    profit_margin = (
        (finance["net_profit"] / finance["total_sales"] * 100)
        if finance["total_sales"] > 0
        else 0
    )
...
def create_decision_agent_executor() -> Tuple[Any, ChatGoogleGenerativeAI]:
    """Create LangChain agent for decision making using modern patterns.

    Returns:
        Tuple of (Agent, LLM instance)
    """
    ...
    # Create agent using the new LangChain 1.2+ pattern
...
@enhanced_traceable(
    name="Inventory Needs Analysis",
    tags=["decision", "inventory", "reorder", "ai-analysis"],
    metadata={"analysis_type": "inventory_needs", "ai_powered": True},
)
def analyze_inventory_needs(region: Optional[str] = None) -> Dict[str, Any]:
    ...
    for item in inventory_info["low_stock_items"]:
        predicted_qty = calculate_reorder_quantity(
    ...
    return {
        ...
        "context": {"inventory": inventory_info, "top_vendors": vendors},
    }


@enhanced_traceable(
    name="Sales Opportunity Analysis",
    tags=["decision", "sales", "opportunity", "ai-analysis"],
    metadata={"analysis_type": "sales_opportunity", "ai_powered": True},
)
def analyze_sales_opportunity(
    category: Optional[str] = None, days: int = 7
) -> Dict[str, Any]:
...
@enhanced_traceable(
    name="Vendor Selection Optimization",
    tags=["decision", "vendor", "optimization", "ai-analysis"],
    metadata={"analysis_type": "vendor_selection", "ai_powered": True},
)
def optimize_vendor_selection(sku: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {"sku": sku, "analysis": result["output"], "top_vendors": vendors[:5]}


@enhanced_traceable(
    name="Financial Health Analysis",
    tags=["decision", "finance", "health", "ai-analysis"],
    metadata={"analysis_type": "financial_health", "ai_powered": True},
)
def analyze_financial_health(region: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {"region": region, "analysis": result["output"], "metrics": finance}
...
def analyze_all_regions(analysis_type: str = "inventory") -> Dict[str, Any]:
    ...
    return {"analysis_type": analysis_type, "results": results}
