...
def load_csv_to_db(table_name: str, csv_file: str, engine, chunk: bool = False) -> int:
    ...
    potential_numeric = ['temperature', 'rainfall', 'humidity', 'revenue', 'amount', 'unit_price', 'unit_cost']
    ...
    if chunk:
        for i in range(0, len(df), CHUNK_SIZE):
            df.iloc[i:i+CHUNK_SIZE].to_sql(table_name, engine, if_exists='append', index=False)
    else:
...
def seed_database():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            conn.execute(text("TRUNCATE TABLE tickets, sales, finance, inventory, vendors, conversations, forecasts CASCADE"))
            conn.commit()
